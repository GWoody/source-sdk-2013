...
#include <vgui_controls/VerticalSlider.h>
...
DECLARE_BUILD_FACTORY( VerticalSlider );
...
//-----------------------------------------------------------------------------
VerticalSlider::VerticalSlider(Panel *parent, const char *panelName ) : BaseClass(parent, panelName)
{
...
//			100 - 
//-----------------------------------------------------------------------------
void VerticalSlider::SetSliderThumbSubRange( bool bEnable, int nMin /*= 0*/, int nMax /*= 100*/ )
{
...
//-----------------------------------------------------------------------------
void VerticalSlider::OnSizeChanged(int wide,int tall)
{
...
//-----------------------------------------------------------------------------
void VerticalSlider::SetValue(int value, bool bTriggerChangeMessage)
{
	int oldValue=_value;
...
//-----------------------------------------------------------------------------
void VerticalSlider::PerformLayout()
{
...
//-----------------------------------------------------------------------------
void VerticalSlider::RecomputeNobPosFromValue()
{
	//int wide,tall;
...
//-----------------------------------------------------------------------------
void VerticalSlider::RecomputeValueFromNobPos()
{
	int value = EstimateValueAtPos( _nobPos[ 0 ], 0 );
	SetValue( value );
}

int VerticalSlider::EstimateValueAtPos( int localMouseX, int /*localMouseY*/ )
{
	int x, y, wide, tall;
	GetTrackRect( x, y, wide, tall );
...
void VerticalSlider::SetInverted( bool bInverted )
{
...
//-----------------------------------------------------------------------------
void VerticalSlider::SendSliderMovedMessage()
{	
...
//-----------------------------------------------------------------------------
void VerticalSlider::SendSliderDragStartMessage()
{	
	// send a message
...
//-----------------------------------------------------------------------------
void VerticalSlider::SendSliderDragEndMessage()
{	
	// send a message
...
// Purpose: 
//-----------------------------------------------------------------------------
void VerticalSlider::ApplySchemeSettings(IScheme *pScheme)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void VerticalSlider::GetSettings(KeyValues *outResourceData)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void VerticalSlider::ApplySettings(KeyValues *inResourceData)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
const char *VerticalSlider::GetDescription()
{
...
//-----------------------------------------------------------------------------
void VerticalSlider::GetTrackRect( int& x, int& y, int& w, int& h )
{
	int wide, tall;
	GetPaintSize( wide, tall );
...
//-----------------------------------------------------------------------------
void VerticalSlider::DrawTickLabels()
{
	int x, y;
	int wide,tall;
	GetTrackRect( x, y, wide, tall );
...
//-----------------------------------------------------------------------------
void VerticalSlider::SetTickCaptions( const char *left, const char *right )
{
	if (left)
	{
		if (_leftCaption)
...
// Purpose: Set the text labels of the Start and end ticks.
//-----------------------------------------------------------------------------
void VerticalSlider::SetTickCaptions( const wchar_t *left, const wchar_t *right )
{
	if (left)
	{
		if (_leftCaption)
...
//-----------------------------------------------------------------------------
void VerticalSlider::PaintBackground()
{
...
//-----------------------------------------------------------------------------
void VerticalSlider::SetRange(int min,int max)
{
	_range[0]=min;
	_range[1]=max;
...
//-----------------------------------------------------------------------------
void VerticalSlider::ClampRange()
{
	if ( _range[0] < _range[1] )
	{
		if(_value<_range[0])
...
//-----------------------------------------------------------------------------
void VerticalSlider::GetRange(int& min,int& max)
{
	min=_range[0];
	max=_range[1];
}
...
// and dragging.
//-----------------------------------------------------------------------------
void VerticalSlider::OnCursorMoved(int x,int y)
{
	if(!_dragging)
	{
		return;
...
// Input  : state - 
//-----------------------------------------------------------------------------
void VerticalSlider::SetDragOnRepositionNob( bool state )
{
	m_bIsDragOnRepositionNob = state;
}

bool VerticalSlider::IsDragOnRepositionNob() const
{
	return m_bIsDragOnRepositionNob;
}

bool VerticalSlider::IsDragged( void ) const
{
	return _dragging;
}
...
//-----------------------------------------------------------------------------
void VerticalSlider::OnMousePressed(MouseCode code)
{
	int x,y;
...
//-----------------------------------------------------------------------------
void VerticalSlider::OnMouseDoublePressed(MouseCode code)
{
	OnMousePressed(code);
}
...
//-----------------------------------------------------------------------------
#ifdef _X360
void VerticalSlider::OnKeyCodePressed(KeyCode code)
{
...
//-----------------------------------------------------------------------------
void VerticalSlider::OnKeyCodeTyped(KeyCode code)
{
	switch (code)
	{
...
//-----------------------------------------------------------------------------
void VerticalSlider::OnMouseReleased(MouseCode code)
{
	if ( _dragging )
	{
		_dragging=false;
...
//-----------------------------------------------------------------------------
void VerticalSlider::GetNobPos(int& min, int& max)
{
	min=_nobPos[0];
	max=_nobPos[1];
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void VerticalSlider::SetButtonOffset(int buttonOffset)
{
	_buttonOffset=buttonOffset;
}

void VerticalSlider::SetThumbWidth( int width )
{
...
//-----------------------------------------------------------------------------
void VerticalSlider::SetNumTicks( int ticks )
{
	m_nNumTicks = ticks;
}
